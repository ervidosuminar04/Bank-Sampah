    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .level-badge {
            ...
            animation: solarPulse 1.8s ease infinite;
        }
...
<body>
    ...
    <div class="card">
        ...
        <h1><span>📈</span> Detail &amp; Riwayat Tabungan</h1>
        <p class="description">
            Halaman ini terproteksi oleh <code>role:nasabah</code> middleware. Kamu berhasil mengakses halaman ini karena masuk sebagai pengguna dengan tipe <strong>Nasabah</strong>.
        </p>

        <div class="info-box">
            <div class="info-title">Detail Tabungan Kamu</div>
            
            <div class="info-row">
                <span class="info-label">Nama Pemilik</span>
                <span class="info-value">{{ $nasabah->nasabah_nama }}</span>
            </div>
            
            <div class="info-row">
                <span class="info-label">Username</span>
                <span class="info-value"><code>{{ $nasabah->nasabah_username }}</code></span>
            </div>

            <div class="info-row">
                <span class="info-label">Nomor Rekening</span>
                <span class="info-value"><code>{{ $nasabah->tabungan->tabungan_no_rekening ?? '-' }}</code></span>
            </div>

            <div class="info-row">
                <span class="info-label">Saldo Akhir</span>
                <span class="info-value saldo-highlight">Rp {{ number_format($nasabah->tabungan->tabungan_saldo_akhir ?? 0, 0, ',', '.') }}</span>
            </div>

            <div class="info-row">
                <span class="info-label">Level Keanggotaan</span>
                <span class="info-value">
                    <span class="level-badge">{{ $nasabah->gamifikasi->level_nasabah ?? 'Bronze' }}</span>
                </span>
            </div>
        </div>
